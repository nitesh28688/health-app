...
9. Phase 19 (2026-07-10, Antigravity, fixed same day by Fable): AI Assistant Feature. A conversational AI was integrated into the app shell to answer questions about users' historical data (totals, trends, workouts) and propose multi-table inserts to repeat workouts with explicit offline and user confirmation checks. Uses Vertex AI function calling, executed securely via an RLS-scoped Supabase client. **Shipped non-functional — two Gemini API-shape bugs (tools array needed a `functionDeclarations` wrapper; function-response payloads can't be bare arrays) made every tool call 400 immediately.** Both fixed and reverified with a real live Vertex round trip. Full detail in `UPGRADE.md` Phase 19 / `STRUCTURE.md` Phase 19.
