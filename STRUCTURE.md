...
- Not yet applied to production at commit time — migration requires the Supabase DB password, which isn't in any env file this session could read (intentional). User is applying `0032_physio.sql` manually via the Supabase SQL Editor; full end-to-end click-testing (which needs a real login) is still pending as of this write-up.
