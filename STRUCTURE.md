...
| 0046 | `0046_boutique_cache.sql` | Adds `boutique_matches` to `wellness_discover_feed_cache` to store 0-credit product recommendations. |
...
**Phase 60 (2026-07-14): Physio/Rehab Mode.** Built the "Adaptive AI Physio/Rehab Mode" originally scoped in the 2026-07-11 Wellness-architecture planning (memory `project_health_app_roadmap`) as Phase 4 of that vision-AI rollout, then never started. User remembered it as "Live AI Physio mode" and asked to check before re-planning — confirmed via `git log`/grep across STRUCTURE/HANDOFF/UPGRADE that nothing had been built, only planned. Live scope decided this session (differs from the original 2026-07-11 sketch in two ways: adaptive from day one instead of one-shot-then-adapt-later, and "live" means a follow-along timer/checklist, not continuous camera analysis):
- **Schema** (`0032_physio.sql`): `physio_exercises` (public curated library — 36 hand-picked bodyweight rehab exercises across knee/shoulder/back/neck/hip/ankle/wrist, `body_area` + `contraindication_notes`), `physio_programs` (one per user complaint, `status` active/resolved), `physio_program_sessions` (one AI-generated routine per session, `exercises` jsonb, `pain_before`/`pain_after`/`difficulty`, `completed_at`). Deliberately a separate table from `exercises` — physio needs `body_area`/`contraindication_notes` fields the general strength/cardio/yoga library doesn't have. `ai_suggestions.kind` check constraint widened to add `physio_plan` (same hard-cap pattern documented in `0026_form_check_cap.sql` — every kind MUST be listed there or the cap silently never enforces).
- **`/api/ai/physio-plan`**: two modes. `initial` (new program: body_area + complaint + optional photo/video) creates the `physio_programs` row. `followup` (existing program) has the SERVER pull the real session history itself — never trusts client-supplied pain/difficulty numbers, so a tampered request can't game the adaptive difficulty. Prompts the model to prefer picking 3-5 exercises from the curated library for that body area (with a fallback to one original AI-generated bodyweight exercise only if the library has nothing suitable), and to bail with a `safety_note` (no exercises generated) if the complaint reads as beyond home-exercise scope. 5/day cap (same tier as form-check, since it's video-capable).
- **Client-side safety gate** (`PhysioSheet.tsx`, before ANY AI call): a red-flag checklist (sudden/severe pain, numbness/tingling, recent trauma, swelling+fever) — any box checked stops the flow with a "see a doctor" message, the AI is never called. This is the primary gate; the model's own `safety_note` escape hatch is the second line of defense, not the first.
- **Flow**: Workout tab gets a new `+ Physio` button (teal, third entry point alongside `+ Log Workout`/`+ Freeform`) → program list (active programs, Continue/Resolve) → intake (body area picker, complaint text, optional photo/video reusing `compressImage`/a trimmed `FormCheckSheet`-style `MediaRecorder` capture) → red-flag checklist → pre-session pain slider → AI generates the routine → session screen (reuses `SetTimer` for `hold_sec` exercises, a simple checkbox for rep-based ones — no new capture tech, per the user's explicit "timer/counter, not continuous camera" scoping) → post-session check-in (pain-after slider + easy/right/hard rating, written to the session row) → back to the program list, which then feeds the next `followup` generation.
- **Assistant wiring** (same build, per the existing feature-lifecycle rule): `open_physio` tool (UI-action proposal, mirrors `check_form`'s pattern exactly — `AppShell.tsx` now renders `PhysioSheet` globally alongside `FormCheckSheet`, both openable from an Assistant proposal card) fires when the user describes a pain/ache in chat; `get_physio_programs` (read-only) lets the Assistant answer questions about program/session history.
- Migration applied to production (2026-07-14, run manually by the user via SQL Editor). First attempt failed on the `ai_suggestions_kind_check` ALTER — the migration was drafted off the check constraint's state as of `0026_form_check_cap.sql`, but `0027`/`0028`/`0029` had widened it further since (added `skin_scan`/`eye_scan`/`hair_scan`/`wellness_insight`) — existing rows using those kinds violated the stale constraint. Fixed by rebuilding the list from `0029` (the actual latest at commit time) instead, pushed as commit `2399f0e`. **Lesson: when re-deriving a check constraint's full value list from a past migration, grep for every later migration that also touched it — the constraint's true current state is the LAST one, not the first one found.** Confirmed live post-fix: 36 exercises seeded across all 7 body areas, all three tables reachable. Full end-to-end click-testing (which needs a real login) is still pending as of this write-up.

**Phase 61 (2026-07-14): Wellness Mode overhaul — scoring calibration, severity fixes, lost-data recovery.**
User asked to make Wellness mode better (functional + UI/UX, scoring, details, extra features). Full read of `wellness/page.tsx` + `wellness-scan/route.ts` + `WellnessCaptureSheet.tsx` + `PDFReportTemplate.tsx` surfaced several real bugs alongside the improvements:
- **`skin_age_estimate` column was STILL missing from production** — migration `0030` was flagged as unapplied two sessions ago and never run; the scan route's PGRST204 fallback had been silently discarding every skin-age estimate since the feature shipped. Migration `0033_wellness_quality.sql` re-adds it (idempotent) plus new `photo_quality` ('good'/'fair'/'poor') and `ai_confidence` ('high'/'medium'/'low') columns. The route's single-column fallback became a loop over all three optional columns, stripping whichever PostgREST names and retrying.
- **Score severity colors were inverted everywhere**: `ScoreRing`, both share-canvas generators, and sub-score displays used `>=80 emerald : >=50 rose : amber` — so a 55 rendered alarming rose while a 30 rendered mild amber. Centralized into `scoreColorClass`/`scoreColorHex`/`scoreBarClass` (emerald ≥80, amber ≥60, rose below) and applied everywhere including the PDF's sub-score bars (previously a fixed rose-violet gradient regardless of value).
- **Routine tab's AM/PM split was `i % 2`** — alternating array indexes, which could put retinol in "Morning" and SPF in "Evening" (actively wrong advice, not just arbitrary). New scans now carry an AI-set `time_of_day` ('am'/'pm'/'both') per recommendation (enforced in the response schema + prompt rules: SPF always am, retinoids/exfoliating acids always pm); old scans fall back to `routineTime()`'s keyword heuristic. 'both' items render in both sections with an "AM + PM" tag; a section left empty says so instead of rendering blank.
- **Scoring calibration rubric added to all three scan prompts** (shared `RUBRIC` template): explicit 5-band anchors (90-100 exceptional … 0-39 significant), "most real scans land 55-85, don't cluster at 70-80", and the key consistency rule — score sub-categories FIRST, then set `overall_score` = mean of sub_scores (previously both were independently generated and could contradict). Fair/poor photos are instructed to score conservatively (45-80 band) with lowered confidence.
- **Capture quality**: `WellnessCaptureSheet` was requesting a 640×480 camera feed — starving skin/hair texture analysis of detail. Now requests 1920×1440 (compressImage still caps the upload at 1024px, so a downscaled high-res capture beats a low-res native one).
- **New zero-AI-cost report details**: Visible Skin Age chip (data existed since 0030-era code but was never rendered anywhere — now in the report header, detailed share canvas, and PDF hero), photo-quality/confidence chip when the photo was less than good, a Score History line chart (last 10 scans of that type, best score highlighted, date range labels) inside the report's Overview tab, and compare mode now shows a category-by-category sub-score delta table (A → B with colored arrows) when both selected scans are the same type — plus an explanatory note when they're not.
Migration `0033` requires a manual SQL Editor run (same as 0032). Verified: tsc clean, /wellness compiles with no server errors; live scan verification needs a real login + an applied migration.

**Phase 62 (2026-07-16): Women's health surfacing + full AI Assistant overhaul.**
Two threads this session, both user-driven off real usage feedback (not planned in advance).

*Women's health* — Menstrual cycle tracking was opt-in via a buried Settings toggle. Migration `0034_womens_health.sql` drops `profiles.track_cycle` (now unconditional) and adds `profiles.conditions text[]` + `cycle_logs.symptom_tags text[]`. Trends now shows a pink "Cycle Tracking" card automatically for `sex === "female"` (next-period prediction via existing `predict_next_period()` RPC). `/cycle` gained symptom-tag chips on the log form and a "Conditions" section (PCOS/PCOD/endometriosis/thyroid chips, `web/lib/womensHealth.ts` holds the tag list + static per-condition tips) that persists to `profiles.conditions`.

*AI Assistant overhaul* — driven by four rounds of real user testing surfacing real bugs, not hypothetical hardening:
- **Personalization** (`0035_ai_personalization.sql`, `profiles.ai_tone`/`ai_name`): Settings gained an "AI Assistant" section (tone: Balanced/Blunt/Gentle/Hype; optional custom name, shown in `AssistantSheet`'s header). `web/lib/aiTone.ts` holds tone → prompt-instruction mapping.
- **Wrong "today"/"past week" answers**: the model had no ground-truth date, so it guessed one for `get_daily_totals`/`get_workout_history` date-range args and routinely landed on an empty range — reported as "you haven't logged anything" despite regular logging. Fixed by computing `today` server-side in IST (Vercel runs UTC, which drifts a day off IST near midnight) and stating it explicitly in the system prompt. Same IST fix applied to the daily 20-turn cap and the repeat-workout `log_date`, which had the identical latent bug.
- **Wrong weekly totals**: separately, even once given the right date range, the model was summing/averaging multi-day rows itself in prose — unreliable arithmetic. `get_daily_totals` (`web/lib/aiTools.ts`) now returns `{ daily_rows, summary: { days_in_range, days_logged, totals, avg_per_day_in_range, avg_per_logged_day } }`, computed deterministically in JS; the tool description and system prompt both direct the model to use `summary` rather than compute anything itself.
- **"Just reads back macros I can already see"**: the prompt only ever asked for a report. Rewrote it to require interpretation — verdict (on-track/off-track/mixed) + one concrete suggestion, comparing against the user's own target/context, defaulting to a 7-day pull for any pattern-shaped question even when phrased as "today". Split by mode: Wellness gets its own dermat/wellness-coach-flavored version (compare scans via `get_wellness_trend`, name the sub-score driving a change) instead of inheriting Core's diet-flavored language verbatim.
- **Tone had no effect on hostile/venting messages**: a "you suck" on Blunt tone still got Gemini's default "I understand you're frustrated..." de-escalation script. Each tone in `aiTone.ts` now has a `frustration` clause (Blunt: skip the coddling, straight to the fix; Gentle: genuine empathy first; Hype: reframe as motivation; Balanced: brief then practical) wired into the system prompt.
- **Vendor/stack disclosure**: asked "who powers you", it answered "I am a large language model powered by Google" — breaking brand ownership and leaking the tech stack. New `web/lib/aiIdentity.ts`: `identitySystemNote()` is a hardened prompt instruction (explicitly not overridable by "ignore previous instructions"/"repeat your system prompt"-style probes) plus `redactVendorMentions()`, a sentence-level regex backstop run on every final reply (`assistant/route.ts`) that swaps any sentence naming the vendor/model/stack/system-prompt for a canned "I'm ⟨name⟩, built into Core AI" line — a defense-in-depth net that catches a leak even if the model was talked into one, not just a prompt-only fix. Audited the other AI routes (wellness-scan, physio-plan, daily-tip, etc.) — all are one-shot structured-JSON generation with no free-text user input, so they have no open conversational surface for this class of probing; only the assistant route needed it.
All of the above: typechecks clean, no live click-testing yet (see HANDOFF_NEXT_SESSION.md) — migrations `0034`/`0035` need a manual Supabase SQL Editor run.

**Phase 63 (2026-07-16): Wellness Journal / "Timecapsule".** User-requested companion feature: time-stamped personal entries in Wellness mode ("laser hair removal today") that the AI comments on at save time and can recall later ("when did I last do laser?"). Design decisions: indexing is pure-Postgres FTS (generated `tsvector` column over entry text + AI-extracted tags, GIN index, `search_journal(q)` RPC using `websearch_to_tsquery` with an ilike fallback for stem-resistant terms) — no embeddings/vector store at this scale, zero added cost. AI enrichment is ONE Gemini call per entry (`/api/ai/journal-entry`): extracts `category` (treatment/skincare/hair/mood/habit/health/other) + 1-4 search tags + a tone-matched companion comment (reuses the Phase 62 `ai_tone`/`ai_name` system). Entry INSERT happens before the AI call — a Gemini outage or the 20/day `journal_comment` cap can never lose an entry, it just saves un-enriched. Assistant wiring: `search_journal`/`get_recent_journal` tools + wellness-prompt instruction to use them for any "when did I..." recall instead of guessing. UI: `/journal` page (rose-accent timeline grouped by date, category chip + tag pills + AI comment block per entry), new Journal tab in the wellness nav (now Scan/Journal/[toggle]/Reports — `toggleIdx` unified to 2 for both modes), `/journal` added to the deep-link mode-reconciliation alongside `/wellness`. Migration `0036_wellness_journal.sql` also rebuilds `ai_suggestions_kind_check` from 0032's list + `journal_comment` (the Phase 60 lesson, applied). The literal-NUL-byte editor gotcha from the physio session recurred while writing the route's `stripNulls` helper — fixed via node script, helper now uses `String.fromCharCode(0)` instead of an escape sequence. v2 deferred: photo attachments. **Journal reminders explicitly rejected by the user ("don't like the hobby cron, we will do it when we make the proper app") — don't re-propose.** Post-live fix: the FTS column had to become trigger-maintained instead of a generated column — `array_to_string()` isn't immutable so Postgres rejects it in generation expressions (42P17, hit on the live migration run).

**Phase 64 (2026-07-16): Products tab — personalized AI ingredient checker.** User wanted a 5th Wellness tab (the real trigger: the nav's C toggle sat off-center with only 4 slots) and rejected the safe options (routine checklist/progress photos/streaks) — pitched and approved "Products": snap a product's INCI label, one Gemini vision call (`/api/ai/product-check`, 10/day `product_check` cap, 20s timeout since vision+long-label reads outrun the default 9s) reads name/brand/type/ingredients/key actives/PAO months and issues a verdict (`good_match`/`use_carefully`/`skip`) personalized against the user's latest scan classification+observations, flagged conditions, recent journal treatments (e.g. laser → hold the AHA), and their CURRENT shelf — `conflicts[]` warns about combinations with products they already own, which generic apps like Yuka/INCIDecoder can't do. Schema: `0037_products.sql` (`wellness_products`, owner-RLS, constraint rebuilt from 0036's list). Expiry: `opened_at` + `pao_months` → client-computed countdown chip (no cron, deliberately — see the reminders rejection above). UI: `/products` page (verdict chip, conflict banners, actives pills, collapsible full INCI list, "Opened today"/"Finished" actions), capture via `<input capture="environment">` + `compressImage` at 1600px (ingredient print is small; selfie default of 1024 loses legibility). Nav: Wellness is now Scan/Journal/[C]/Products/Reports — toggle centered at index 2, matching Core; `/products` added to deep-link mode reconciliation. Assistant: `get_products` tool + prompt instruction to consult the shelf before ingredient advice. Entry insert happens only after a successful AI read (unlike Journal — a product row without analysis is useless, whereas a journal entry without a comment is still a journal entry).

**Phase 65 (2026-07-16, later): Products — preview-before-save + typed entry.** Two real gaps found immediately on first real use: every scan auto-saved to the shelf with no way to just check a product without keeping it (wrong for "deciding whether to buy" or a mis-scan), and there was no fallback when the camera/label-read failed. Fixed both: `/api/ai/product-check` no longer inserts a row — it returns the analysis as a preview only, and `/products` shows it with explicit "Add to my kit" (does the actual insert client-side, no second AI call — the analysis already ran) / "Discard" actions. The route now also accepts `{ productName, ingredientsText? }` as an alternative to `imageDataUrl` — a "Scan not working? Type it in instead" toggle reveals a name + optional-ingredients form. Prompt branches: typed ingredients are used directly (same accuracy as a photo read), no ingredients means the model falls back to its own general knowledge of that specific product but is explicitly told to say "I don't confidently recognize this" rather than inventing an ingredient list for an unfamiliar product — same "widen general-knowledge permission, don't hardcode" principle as the assistant's Red Bull/cigarette fix.

**Phase 66 (2026-07-16, later): get_products can recall finished/removed products.** User confirmed "Finished" (soft-remove, row stays in DB) is the right model for Products specifically *because* they want to ask the AI about past products later — but `get_products` only ever queried `status='active'`, so that promise wasn't actually kept. Added an `include_finished` boolean param (default false, so the existing "don't suggest a conflicting/owned ingredient" behavior is unchanged); the wellness system prompt now passes `include_finished: true` for any backward-looking question ("what did I used to use", "have I tried X before").

**Phase 67 (2026-07-16, later): Products entry UX simplified.** User pushback on Phase 65's UI: a single "Check a product" button plus an apologetic "Scan not working? Type it in instead" sub-link implied typing was a degraded fallback, and buried it behind an extra click. Replaced with two equal-weight buttons shown immediately (Scan / Type), no framing of one as a workaround for the other. Added a close (✕) on the typed-entry panel since revealing it is now a one-way action (`setShowTyped(true)`) rather than a toggle.

**Phase 68 (2026-07-17): Separate assistant name per mode.** User caught that setting an AI Assistant name in Settings applied to both Core and Wellness — picking "Coach" made the Wellness Assistant "Coach" too, even though tone/name was pitched in Phase 62 as personalizing "how Core and Wellness Assistant talk to you" (implying two identities). Added `profiles.ai_name_wellness` (`0038_ai_name_wellness.sql`) alongside the existing `ai_name` (now implicitly Core-only). Settings shows two name inputs. Wired through: `/api/ai/assistant` picks `ai_name_wellness` vs `ai_name` by `mode`, `/api/ai/journal-entry` (always wellness) now reads `ai_name_wellness`, and `AppShell`'s `AssistantSheet` prop picks the field by the current mode instead of always passing `profile.ai_name`.

**Phase 69 (2026-07-17, later): Products — button copy + web search grounding for typed entries.** Two user-reported issues on `/products`. (1) The Phase 67 button labels "Scan"/"Type" read as unfinished placeholder text — relabeled to "Scan Label" / "Enter Manually" (`web/app/products/page.tsx`). (2) Typing in a product name with no ingredients (e.g. "Nanoliss Quinoa Shampoo," a real search-indexed product) returned "I don't have reliable ingredient data for this exact product" because `/api/ai/product-check` only ever asked Gemini from its training-data memory — no web access. Added `searchGrounded()` to `lib/gemini.ts`: a plain-text (no responseSchema, since forced-JSON mode and Search grounding aren't reliable together) call to `generateChatWithTools` with the `google_search` tool. `product-check/route.ts` now runs this grounded lookup first whenever the user typed a product name but no ingredients, and feeds the search result into the main structured-analysis prompt as context; falls back to the old "use general knowledge, admit if unsure" behavior if the search finds nothing or fails (best-effort, never blocks the main call). Scan-a-photo path is unchanged — the label is already in hand, no search needed.
...
**Phase 75 (2026-07-17, later still): Core-mode usability flag on food-photo estimates.** User asked directly whether Core mode needed an equivalent to Phase 74's `is_usable` gate — good catch, it did, just narrower in scope than Wellness (Core's diet/workout data is structured facts read via aggregate RPCs like `get_daily_totals`, not freeform text a model reads as advice-context, so FIX 1/FIX 2 genuinely don't apply — only the usability signal does, and only to `photo-estimate/route.ts`, the one Core-mode AI call with the same "low-confidence read gets treated as ground truth" shape as product-check's blurry-label case). Explicit scope decision (asked directly, not assumed): flag-only — a low-confidence photo estimate still counts toward daily kcal/protein totals, no silent exclusion from numbers the user is tracking against a target (excluding was considered and rejected: could make the visible total mysteriously drop below what was actually logged). Migration `0042_food_usability.sql` adds `foods.is_usable`; `photo-estimate/route.ts`'s schema gains `is_usable` (false when the photo is too blurry/dark/obscured to confidently estimate); `add/page.tsx` stores it on the AI-created `foods` row and shows a one-time toast on a low-confidence save; `app/page.tsx` (Diary) shows a persistent amber "low confidence" badge next to the food name. Text-based `food-estimate/route.ts` deliberately left untouched — it's backed by a *global* cross-user cache (`ai_food_cache`), a meaningfully different data shape than the per-row photo case, and wasn't part of the scope the user approved.
...
**Phase 76 (2026-07-18): Gemini API Tool Schema Fix & Premium Nav Bar Mode Toggle**
...
- **Premium Mode Toggle Cutout:** Replaced the flat `border-t` background of the bottom navigation bar in `AppShell.tsx` with a curved cutout (moat/cradle) for the center mode toggle button. Implemented using CSS `mask-image` with a perfectly smoothed Bezier curve and `linear-gradient` to allow `backdrop-blur-xl` to apply only to the masked area, leaving a transparent gap around the button. The top edge stroke is drawn via an absolute positioned SVG path tracing the exact curve.
- **Icon Swap & Shimmer:** Swapped the text ("W" or "C") on the mode toggle button for larger (`w-8 h-8`) premium Lucide icons (`Heart` and `Dumbbell`). Replaced the text-clip pulse animation with a premium, whole-button CSS `shimmer` animation (a sweeping light beam using `mix-blend-overlay` and a rotated gradient) to draw the eye subtly.
...
**Phase 81 (2026-07-24): Discover Feed & Boutique 0-Credit Architecture**
- **Zero-Credit Loading:** The Discover Feed (`api/ai/discover-feed`) and Boutique matches (`api/ai/boutique-matches`) now cost 0 AI credits to load. The Discover feed purely fetches RSS articles and mixes them with cached AI tips, while Boutique fetches directly from the cache.
- **Scan-time Generation:** Personalized AI discover tips, protocols, and boutique matches are now generated for "free" concurrently alongside the standard wellness scan in `api/ai/wellness-scan`, then stored in a `wellness_discover_feed_cache` table.
