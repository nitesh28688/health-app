...
| `/wellness` | Contains an aggregate Wellness Score progress ring card at the top averaging usable scan types, showing active inputs, alongside a month-to-date scan count statistic and a programmatic share card canvas action. Tabswitcher for Skin, Eye, and Hair Analysis; month-grouped history grids; taps open detail sheets with overall score and sub-scores progress bars, previous scan trend delta (+X/-X), active unbranded ingredient recommendations (filtered by skin/hair types), photo details, and persistent medical disclaimers; taps on two scans starts side-by-side Before/After comparison. guided capture via MediaPipe Face Landmarker (face/eye scans) and Hair Segmenter (hair scans). |
