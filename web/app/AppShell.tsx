...
function NavTabs({ mode, onModeToggle }: { mode: AppMode; onModeToggle: () => void }) {
  ...
  return (
    <div className="max-w-md mx-auto flex px-2 py-1 relative z-10">
      {items}
      <div className="absolute left-1/2 top-[-26px] -translate-x-1/2 pointer-events-none z-20">
        <button
          onClick={() => {
            hapticTap();
            onModeToggle();
          }}
          className={`relative w-[68px] h-[68px] rounded-full ${destBg} text-white flex items-center justify-center active:scale-95 transition-all duration-200 pointer-events-auto overflow-hidden border border-white/20`}
