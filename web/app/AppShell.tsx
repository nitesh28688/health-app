...
import { Bot, Book, Dumbbell, TrendingUp, Users, Smile, Salad, CloudUpload, Sparkles } from "lucide-react";
...
// Wellness Mode's own tab set — deliberately just 2 tabs. Only one real
// Wellness page exists today (/wellness, with its own internal Skin/Eye/Hair
// switcher + history) — padding this with tabs that'd all point to the same
// page would be hollow navigation. "Profile" doubles as the way back to Core
// Mode, same toggle either direction.
const WELLNESS_TABS = [
  { href: "/wellness", label: "Scan", icon: Sparkles },
  { href: "/profile", label: "Profile", icon: Smile },
];
      ...
      <nav className={`fixed bottom-0 inset-x-0 border-t backdrop-blur-xl pb-[env(safe-area-inset-bottom)] z-50 transition-colors duration-300 ${
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="max-w-md mx-auto flex px-2 py-1"
          >
            {activeTabs.map((t) => {
              const Icon = t.icon;
